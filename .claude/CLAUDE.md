...
crates/
  ...
  doge-compiler/    # EXISTS (M5): keywords (single source of truth), lexer/ (mod+scan+strings), parser/ (mod+stmt+expr), AST + dump (ast.rs — Stmt::span/Expr::span), semantic checks (check/ — mod+stmt+scopes), diagnostics (diagnostics.rs — Diagnostic + source_line helper), stdlib module table, module loader (modules/ — mod+diag), Rust codegen (codegen/ — mod+names+analysis+callable+stmt+expr+calls+dispatch). Each big pass is a directory module: mod.rs holds the driver + shared structs, siblings hold impl-method groups (pub(super)), tests.rs holds its tests
