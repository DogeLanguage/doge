...
crates/
  ...
  doge-cli/         # EXISTS: `doge` binary — hand-rolled args, bark/build/check/fmt/lsp/repl subcommands (repl.rs is the interactive loop; lsp calls doge-lsp; DOGE_INTERP env runs a file through doge-interp), build cache (cache.rs, salted with crate version + codegen rev + a hash of doge-runtime source from build.rs) + cargo build glue (src/build.rs)
